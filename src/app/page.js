...
import { LogIn, LogOut, BookOpen, GraduationCap, Users, Eye, EyeOff, Settings, ArrowUp, ArrowDown } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const savedOrder = localStorage.getItem("autograder_course_order");
    if (savedOrder) {
      try {
  ...
  const toggleCourseVisibility = (e, courseId) => {
    ...
    localStorage.setItem("autograder_hidden_courses", JSON.stringify(newHidden));
  };
  ...
  const moveCourse = (e, index, direction) => {
    ...
    localStorage.setItem("autograder_course_order", JSON.stringify(newOrder));
  };

  if (status === "loading") {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="animate-pulse text-slate-500 font-medium text-lg">Loading...</div>
      </div>
    );
  }
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gradient-to-br from-blue-50 to-indigo-50">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
          <div className="p-8 text-center space-y-6">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">AutoGrader AI</h1>
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col hidden md:flex">
        ...
        <div className="flex-1 p-4 space-y-2">
          <div className="flex items-center gap-3 px-3 py-2 text-slate-600 font-medium rounded-lg bg-indigo-50 text-indigo-700">
      ...
      <main className="flex-1 overflow-auto">
        <header className="h-16 flex items-center justify-between px-8 bg-white/50 backdrop-blur-sm border-b border-transparent sticky top-0 z-10 transition-all duration-200">
          <h2 className="text-xl font-bold text-slate-800">Dashboard</h2>
          {courses.length > 0 && (
            <button
              onClick={() => setShowHidden(!showHidden)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${showHidden ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            >
        ...
        <div className="p-8">
          ...
          {loadingCourses ? (
            ...
            <div className="text-center py-20 bg-white rounded-2xl border border-slate-200 shadow-sm">
              ...
              <p className="text-slate-500">We couldn't find any active Google Classroom courses where you are a teacher.</p>
            </div>
          ) : (
                ...
                .map((course) => {
                  ...
                  return (
                    <div
                      ...
                      className={`bg-white rounded-2xl p-6 border border-slate-200 shadow-sm transition-all relative overflow-hidden group ${isHidden ? 'opacity-50 grayscale select-none' : 'hover:shadow-md cursor-pointer'}`}
                    >
                      {!isHidden && <div className="absolute top-0 left-0 w-full h-1 bg-indigo-500 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform"></div>}
                      <div className="flex justify-between items-start mb-4">
                        <div className="w-12 h-12 bg-indigo-50 text-indigo-700 rounded-xl flex items-center justify-center font-bold text-xl uppercase">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <div className="flex bg-slate-100 rounded-lg border border-slate-200 overflow-hidden">
                            <button
                              ...
                              className="p-1 hover:bg-slate-200 text-slate-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors border-r border-slate-200"
                              title="Move Up"
                            >
                            ...
                            <button
                              ...
                              className="p-1 hover:bg-slate-200 text-slate-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                              title="Move Down"
                            >
                      ...
                      <h3 className="font-bold text-lg text-slate-900 mb-1 line-clamp-1" title={course.name}>{course.name}</h3>
                      <div className="flex items-center gap-2 text-sm text-slate-500 mb-5">
                        <Users className="w-4 h-4" />
                        <span>Google Classroom</span>
                      </div>

                      <div className="pt-4 border-t border-slate-50 flex justify-between items-center group-hover:border-slate-100">
                        <span className="text-xs font-medium text-slate-500 bg-slate-50 px-2 py-1 rounded-md">ID: {course.id}</span>
                        {!isHidden && (
